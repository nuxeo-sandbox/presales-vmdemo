...
# At this point, you can SSH into your target EC2 using the new FQDN; however,
# the services and conf files on the target EC2 are still using the old FQDN.

# In EC2:
# Run this script as root.
# It must be run with the new DNS name as an argument (e.g. sudo ./newDNS.sh myNewDNSname)
...
# It is good practice to delete the old certificate. You will be prompted to do
# so, but not forced.
...
#     -- new DNS name is passed as a command line argument (required)
...
# or you can run `curl https://<new-dns-name>.cloud.nuxeo.com/nuxeo/runningstatus\?info\=started`
...
# New DNS name is required as an argument
if [ -z "$1" ]; then
    echo "Usage: $(basename "$0") <new-dns-name>"
    exit 1
fi

cat << EOF

NOTE: FQDN is just <dns-name>.cloud.nuxeo.com
